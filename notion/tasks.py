...
class Task(RecordBase):
    ...
    _column_names: ClassVar[Set[str]] = {'date_created', 'done', 'due_date', 'last_edited_time', 'name', 'priority', 'schedule', 'status', 'tags'}
    ...
    def deserialize_values(cls, values: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        if 'Date created' in values:
            new_values['date_created'] = dateutil.parser.isoparse(values['Date created']["created_time"])
        if 'Done' in values:
            new_values['done'] = values['Done']["checkbox"]
        if 'Status' in values:
            new_values['status'] = Status[enum_name_to_alias(values['Status']["select"]["name"])]
        if 'Schedule' in values:
            new_values['schedule'] = values['Schedule']["rich_text"][0]["plain_text"]
        if 'Priority' in values:
            new_values['priority'] = Priority[enum_name_to_alias(values['Priority']["select"]["name"])]
        if 'Tags' in values:
            new_values['tags'] = [Tags[enum_name_to_alias(t["name"])] for t in values['Tags']["multi_select"]]
        if 'Due date' in values:
            new_values['due_date'] = dateutil.parser.isoparse(values['Due date']["date"]["start"])
        if 'Last edited time' in values:
            new_values['last_edited_time'] = dateutil.parser.isoparse(values['Last edited time']["last_edited_time"])
        if 'Name' in values:
    ...
    def serialize_values(cls, values: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        if 'schedule' in values:
            new_values['Schedule'] = {
                "type": "rich_text",
                "rich_text": [{"type": "text", "text": {"content": values['schedule']}}],
            }
        if 'priority' in values:
            new_values['Priority'] = {
                "type": "select",
